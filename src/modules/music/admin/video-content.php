...
use NukeViet\Module\music\Shared\Videos;
...
if ($nv_Request->isset_request('submitform', 'post')) {
    ...
    /*
     * Khi thêm video, lưu lại danh sách ca sĩ, nhạc sĩ để thêm nhanh vào lần sau
     * Khi sửa video không lưu lại
     */
    if ($new_video_id) {
        $nv_Request->set_Cookie($module_data . '_mv_last_singers', json_encode($array['singer_ids']), NV_LIVE_COOKIE_TIME);
        $nv_Request->set_Cookie($module_data . '_mv_last_authors', json_encode($array['author_ids']), NV_LIVE_COOKIE_TIME);
    }
...
$last_picked_singers = $nv_Request->get_string($module_data . '_mv_last_singers', 'cookie', '');
$last_picked_authors = $nv_Request->get_string($module_data . '_mv_last_authors', 'cookie', '');
