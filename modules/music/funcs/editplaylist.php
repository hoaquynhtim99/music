...
{
	$pldata = array();
	$row['name'] = $pldata['name'] = $nv_Request->get_title( 'name', 'post', '' );
	$row['keyname'] = $pldata['keyname'] = change_alias( $pldata['name'] );
	$row['singer'] = $pldata['singer'] = $nv_Request->get_title( 'singer', 'post', '' );
	$row['message'] = $pldata['message'] = nv_br2nl( $nv_Request->get_textarea( 'message', '', NV_ALLOWED_HTML_TAGS ) );

	$result = $db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_playlist SET
		name = " . $db->quote( $pldata['name'] ) . ",
		keyname = " . $db->quote( $pldata['keyname'] ) . ",
		singer = " . $db->quote( $pldata['singer'] ) . ",
		message = " . $db->quote( $pldata['message'] ) . "
	WHERE id =" . $id );
