...
if (!nv_function_exists('nv_block_chart_albums')) {
     ...
     * @return
     */
    function nv_block_config_chart_albums($module, $data_block, $lang_block)
    {
        global $site_mods;
    ...
    /**
     * nv_block_config_chart_albums_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_chart_albums_submit($module, $lang_block)
    {
        global $nv_Request;
    ...
    /**
     * nv_block_chart_albums()
     *
     * @param mixed $block_config
     * @return
     */
    function nv_block_chart_albums($block_config)
    {
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_theme . '/block.chart_albums.tpl')) {
            $block_theme = $global_config['module_theme'];
        } else {
            $block_theme = 'default';
        }
        $xtpl = new XTemplate('block.chart_albums.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $module_theme);
        ...
        $cacheFile = NV_LANG_DATA . '_bchartalbums_' . $block_config['bid'] . '_' . implode('_', $block_config['catids']) . '_' . NV_CACHE_PREFIX . '.cache';
        $cacheTTL = 600; // Cache 10 phút
        $id_ajax = 0;
        if ($nv_Request->isset_request('getBlockChartAlbumTab', 'post')) {
            ...
            foreach ($global_array_cat_chart as $cat_chart) {
                if (in_array($cat_chart['cat_id'], $block_config['catids'])) {
                    ...
                    if ($number_tabs == 1) {
                         ...
                         */
                        $array_select_fields = nv_get_album_select_fields(false, 'tb2');
                        $where = [
                            "tb2.status=1",
                            "tb2.is_official=1",
                            "tb1.object_name='album'",
                        ...
                        $db->join("INNER JOIN " . Resources::getTablePrefix() . "_albums tb2 ON tb1.object_id=tb2.album_id");
                        $db->where(implode(' AND ', $where));
                        $db->order('summary_scores DESC');
                        $db->limit($block_config['numrows']);
                        $sql = $db->sql();

                        $array_albums = $array_singer_ids = $array_singers = [];
                        ...
                        if (empty($array_albums)) {
                            ...
                            WHERE chart_time=" . $chart_before . " AND cat_id=" . $cat_chart['cat_id'] . " AND object_name='album' AND object_id IN(" . implode(',', array_keys($array_albums)) . ")";
                            ...
                            foreach ($array_albums as $id => $row) {
                                ...
                                $row['album_link_full'] = NV_MY_DOMAIN . nv_url_rewrite($row['album_link'], true);
                                ...
                                $order_offset_value = isset($data_chart_before[$row['album_id']]) ? ($data_chart_before[$row['album_id']] - $row['chart_order']) : 0;
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$block_config['module']])) {
        $content = nv_block_chart_albums($block_config);
    }
}
