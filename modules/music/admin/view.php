...
if ($area == MS_COMMENT_AREA_SONG) {
    ...
    $sql = "SELECT " . implode(', ', $array_select_fields[0]) . " FROM " . Resources::getTablePrefix() . "_songs WHERE status=1 AND song_id=:song_id";
    $sth = $db->prepare($sql);
    ...
    $sql = "SELECT " . implode(', ', $array_select_fields[0]) . " FROM " . Resources::getTablePrefix() . "_videos WHERE status=1 AND video_id=:video_id";
    $sth = $db->prepare($sql);
    ...
    $sql = "SELECT " . implode(', ', $array_select_fields[0]) . " FROM " . Resources::getTablePrefix() . "_albums WHERE status=1 AND album_id=:album_id";
    $sth = $db->prepare($sql);
    ...
    $sql = "SELECT " . implode(', ', $array_select_fields[0]) . " FROM " . Resources::getTablePrefix() . "_user_playlists WHERE playlist_id=:playlist_id";
    $sth = $db->prepare($sql);
