...
{	
	...
	foreach ( $albumdata as $data => $null )
	{
		...
		if	($null == ''& $data !="thumb") $error = $lang_module['singer_error']; 
	}
	if ( $error == "" )
	{
		$query = "INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data . "_author` 
		(
			`id`, `ten`, `tenthat`, `thumb`, `introduction`, `numsong`, `numvideo`
		) 
		VALUES 
		( 
			NULL, 
