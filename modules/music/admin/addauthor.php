...
if ( ($nv_Request->get_int( 'edit', 'post', 0 )) == 1 )
{
	...
	if( empty( $error ) )
	{
		...
		if ( $result ) 
		{
			if( $row['ten'] != $array['ten'] ) updatewhendelA( $row['ten'], $array['ten'] ); // Cap nhat lai nhac si cua cac bai hat neu thay doi ten ngan gon
			nv_del_moduleCache( $module_name );
			
			Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=author" ); die();
		}
		else
		{
