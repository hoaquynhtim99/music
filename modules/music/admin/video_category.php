...
// Page title collum
$page_title = $lang_module['sub_videocategory'];
...
$i = 1;
while( list( $id, $title, $keywords, $description, $numvideo, $weight ) = $result->fetch( 3 ) )
{
	$list_weight = array();
	for( $j = 1; $j <= $num; $j++ )
	{
...
if( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	else
	{
		if( empty( $id ) )
		{
			$sql = "SELECT id FROM " . NV_PREFIXLANG . "_" . $module_data . "_video_category WHERE title=" . $db->quote( $array['title'] );
			$result = $db->query( $sql );
			...
			else
			{
				...
				if( $db->insert_id( $sql ) )
				{
					//$xxx->closeCursor();
					$nv_Cache->delMod( $module_name );
					nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['vcat_add'], $array['title'], $admin_info['userid'] );
		...
		{
			$sql = "SELECT id FROM " . NV_PREFIXLANG . "_" . $module_data . "_video_category WHERE title=" . $db->quote( $array['title'] ) . " AND id!=" . $id;
			$result = $db->query( $sql );
			$check_exist = $result->fetchColumn();
			...
			else
			{
				...
				if( $db->query( $sql ) )
				{
					//$xxx->closeCursor();
					$nv_Cache->delMod( $module_name );
					nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['vcat_edit'], $array_old['title'] . "&nbsp;=&gt;&nbsp;" . $array['title'], $admin_info['userid'] );
					Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op );
					exit();
				}
				else
...
$xtpl = new XTemplate( "music_vcat.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file );
