...
$sqlnum = "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_author WHERE ".$where."";
...
$contents .= "<label>" . $lang_module['author_search'] . ": </label><br />\n";
...
while($rs = $db->sql_fetchrow($result))
{
	...
	$xtpl->assign('URL_SONG', "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&type_search=nhacsi&q=" . $rs['ten'] );
	$xtpl->assign('URL_VIDEO', "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=videoclip&type_search=nhacsi&q=" . $rs['ten'] );

	$xtpl->assign('url_add_song', "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=addsong&nhacsi=".$rs['ten'] );
	
	$class = ($i % 2) ? " class=\"second\"" : "";
	$xtpl->assign('class', $class);
	$xtpl->assign('URL_DEL_ONE', $link_del . "&where=_author&id=" . $rs['id']);
