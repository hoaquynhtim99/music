...
$link = "index.php?" . NV_NAME_VARIABLE . "=" . $module_name."&op=commentalbum" ;
...
// tinh so trang
$num_page = "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data."_comment_album" ;
...
$xtpl->assign('URL_DEL_BACK', "index.php?" . NV_NAME_VARIABLE . "=" . $module_name."&op=commentalbum");
$xtpl->assign('URL_DEL', "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=delall&where=_comment_album");

$sql = "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_comment_album ORDER BY ID DESC LIMIT ".$first_page.",100";
...
{	
	...
	$sql = "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_album WHERE id=".$rs['what']." LIMIT 0,1";
	$resultsong = mysql_query( $sql );	
	while ($song = $db->sql_fetchrow($resultsong))
	{
		$albumname = $song['tname'];
	}
	$xtpl->assign('album', $albumname);
	
	$class = ($i % 2) ? " class=\"second\"" : "";
	$xtpl->assign('class', $class);
	$xtpl->assign('URL_DEL_ONE', $link_del . "&where=_comment_album&id=" . $rs['id']);
	$xtpl->parse('main.row');
}
