...
$repository_url = "http://nukeviet-music.googlecode.com/svn/trunk/";
define( 'NV3_MUSIC_DIRECTORY_SNV', '/trunk/' );
...
{
    return preg_replace( "/^" . nv_preg_quote( NV3_MUSIC_DIRECTORY_SNV ) . "(.*)$/", "\\1", $path );
}
...
$vini = isset( $module_setting['revision'] ) ? $module_setting['revision'] : 0; // Phien ban truoc
if ( $vini < 141 )
{
...
else
{
    ...
    if ( $step == 1 )
    {
        ...
        if ( $vend > $vini )
        {
            ...
            $logs = $svn->getFileLogs( NV3_MUSIC_DIRECTORY_SNV, $vini, $vend );
            if ( ! empty( $logs ) )
            {
                ...
                else
                {
                    ...
                    file_put_contents( NV_ROOTDIR . '/' . NV_DATADIR . '/svn_music_data_file' . md5( $module_setting['revision'] . $global_config['sitekey'] ) . '.log', serialize( $svn_data_files ), LOCK_EX );
    ...
    {
        if ( file_exists( NV_ROOTDIR . '/' . NV_DATADIR . '/svn_music_data_file' . md5( $module_setting['revision'] . $global_config['sitekey'] ) . '.log' ) )
        {
            $cache = file_get_contents( NV_ROOTDIR . '/' . NV_DATADIR . '/svn_music_data_file' . md5( $module_setting['revision'] . $global_config['sitekey'] ) . '.log' );
            $svn_data_files = unserialize( $cache );
            $check_del_dir_update = true;
            if ( is_dir( NV_ROOTDIR . '/install/updatemusic' ) )
            {
                $del_error = nv_deletefile( NV_ROOTDIR . '/install/updatemusic', true );
            ...
            {
                if ( ! nv_mkdir_svn( 'install/updatemusic' ) )
                {
                    $contents = sprintf( $lang_global['error_create_directories_failed'], 'install/updatemusic' );
                    if ( $sys_info['ftp_support'] and intval( $global_config['ftp_check_login'] ) != 1 )
                    {
                        $contents .= '<br /><br />' . $lang_module['revision_config_ftp'];
                    }
                ...
                {
                    nv_mkdir_svn( 'install/updatemusic/new' );
                    nv_mkdir_svn( 'install/updatemusic/old' );
                    
                    @file_put_contents( NV_ROOTDIR . "/install/updatemusic/.htaccess", "deny from all", LOCK_EX );
                    @file_put_contents( NV_ROOTDIR . "/install/updatemusic/index.html", "", LOCK_EX );
                    @file_put_contents( NV_ROOTDIR . "/install/updatemusic/new/index.html", "", LOCK_EX );
                    @file_put_contents( NV_ROOTDIR . "/install/updatemusic/old/index.html", "", LOCK_EX );
    ...
    elseif ( $step == 3 )
    {
        ...
        if ( $getfile < count( $download_files ) )
        {
            ...
            $path = NV3_MUSIC_DIRECTORY_SNV . $file_name;
            
            // download new file
            $fileInfo = $svn->getDirectoryTree( $path, $vend, false );
            $dirname = 'install/updatemusic/new/' . del_path_svn( $path );
            ...
            if ( ! empty( $filename ) and ! empty( $dirname ) )
            {
                ...
                if ( in_array( $file_name, $svn_data_files['edit_files'] ) )
                {
                    ...
                    {
                        $filename = basename( $path );
                        $dirname = 'install/updatemusic/old/' . del_path_svn( $path );
                        $dirname = substr( $dirname, 0, - ( strlen( $filename ) + 1 ) );
                        if ( ! empty( $dirname ) and ! is_dir( NV_ROOTDIR . '/' . $dirname ) )
                        {
                            if ( ! nv_mkdir_svn( $dirname ) )
        ...
        {
            $path = NV3_MUSIC_DIRECTORY_SNV . "update_module_music.php";
            $contents_f = $svn->getFile( $path, $vend );
            ...
            file_put_contents( NV_ROOTDIR . "/install/updatemusic/update.php", $contents_f, LOCK_EX );
            nv_deletefile( NV_ROOTDIR . '/' . NV_DATADIR . '/svn_music_data_file' . md5( $module_setting['revision'] . $global_config['sitekey'] ) . '.log' );
