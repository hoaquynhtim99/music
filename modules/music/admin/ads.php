...
if ( $nv_Request->get_int( 'add', 'post', 0 ) == 1 )
{
	...
	else
	{
		...
		{ 
			...
			Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name."&". NV_OP_VARIABLE ."=ads" ); die();
		} 
		else 
		{ 
	...
	<tr>
		...
		<td align=\"center\">
			<span class=\"delete_icon\">
				<a class='delfile' href=\"index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=delads&id=".$row['id']."\">".$lang_module['delete']."</a>
			</span>
		</td>
	</tr>";
	$i ++ ;
...
<table class=\"tab1\">
	<thead>
		<tr>
			<td colspan=\"2\">Thêm quảng cáo mới</td>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td style=\"background: #eee;\">Tên quảng cáo</td>
			<td style=\"background: #eee;\">
				<input style=\"width:400px;\" type=\"text\" name=\"name\" value=\"" . $name . "\" />
			</td>
		</tr>
		<tr>
			<td style=\"background: #eee;\">Đường dẫn</td>
			<td style=\"background: #eee;\">
		...
		$('a[class=\"delfile\"]').click(function(event)
		{
			...
			if (confirm(\"".$lang_module['ads_del_confirm']."\"))
			{
				...
				$.ajax(
				{
					...
					{
						alert(data);
						window.location = 'index.php?" . NV_NAME_VARIABLE . "=" . $module_name."&op=ads';
					}
				});
			}
		});
