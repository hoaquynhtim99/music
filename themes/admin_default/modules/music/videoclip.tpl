		...
		<tr>
			<td>
				<form id="filter-form" method="get" action="" onsubmit="return false;">
					<input class="music-input text size1" type="text" name="q" value="{DATA_SEARCH.q}" placeholder="{LANG.video_name}"/>
	...
	<table class="tab1">
		...
		<tbody{ROW.class}>
			<tr class="topalign">
				...
				<td class="center"><a href="{ROW.thumb}" title="{ROW.title}" rel="shadowbox"><img src="{ROW.thumb}" alt="{ROW.title}" width="84" height="50"/></a></td>
				<td>{ROW.title}</td>
				<td>{ROW.singers}</td>
				<td>{ROW.authors}</td>
				<td>{ROW.theloai}</td>
