...
<form action="{FORM_ACTION}" method="post">
	<table class="tab1">
		...
		<tfoot>
			<tr>
				<td class="center" colspan="2"><input type="submit" class="music-button" name="submit" value="{LANG.save}"/></td>
			</tr>
		</tfoot>
	</table>
</form>
...
$(document).ready(function(){
	...
	$("a#addonevideo").click(function(){
		...
		nv_open_browse_file( "{NV_BASE_ADMINURL}index.php?" + nv_name_variable + "=" + nv_module_name + "&" + nv_fc_variable + "=videoclip&findOneAndReturn=1&area=listvideos-area&input=videos&listvideo=" + listvideo, "NVImg", "850", "600", "resizable=no,scrollbars=no,toolbar=no,location=no,status=no" );
	});
	$("a#addlistvideo").click(function(){
		var listvideo = $("input[name=videos]").attr("value");
		nv_open_browse_file( "{NV_BASE_ADMINURL}index.php?" + nv_name_variable + "=" + nv_module_name + "&" + nv_fc_variable + "=videoclip&findListAndReturn=1&area=listvideos-area&input=videos&listvideo=" + listvideo, "NVImg", "850", "600", "resizable=no,scrollbars=no,toolbar=no,location=no,status=no" );
	});

	$("a#addonealbum").click(function(){
		var listalbum = $("input[name=albums]").attr("value");
		nv_open_browse_file( "{NV_BASE_ADMINURL}index.php?" + nv_name_variable + "=" + nv_module_name + "&" + nv_fc_variable + "=album&findOneAndReturn=1&area=listalbums-area&input=albums&listalbum=" + listalbum, "NVImg", "850", "600", "resizable=no,scrollbars=no,toolbar=no,location=no,status=no" );
	});
	$("a#addlistalbum").click(function(){
		var listalbum = $("input[name=albums]").attr("value");
		nv_open_browse_file( "{NV_BASE_ADMINURL}index.php?" + nv_name_variable + "=" + nv_module_name + "&" + nv_fc_variable + "=album&findListAndReturn=1&area=listalbums-area&input=albums&listalbum=" + listalbum, "NVImg", "850", "600", "resizable=no,scrollbars=no,toolbar=no,location=no,status=no" );
	});
});
</script>
<!-- END: main -->